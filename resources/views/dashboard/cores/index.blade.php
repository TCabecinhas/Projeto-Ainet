            ...
            <tbody>
                @foreach ($cores as $c)
                    <tr>
                        <td>{{ $c->code }}</td>
                        <td>{{ $c->name }}</td>
                        <td><span class="dot" style="background-color: #{{ $c->code }}"></span></td>
                        <td>
                            {{-- Editar --}}
                            <a href="{{ route('dashboard.cores.edit', $c) }}" class="btn btn-sm btn-outline-warning"><i
                                    class="fa fa-pencil-alt"></i></a>

                            {{-- Eliminar --}}
                            <button type="submit" form="form_destroy_{{ $c->code }}"
                                class="btn btn-sm btn-outline-danger"><i class="fa fa-trash"></i></button>
                            <form action="{{ route('dashboard.cores.destroy', $c) }}" id="form_destroy_{{ $c->code }}"
                                method="POST">
                                @csrf
                                @method('DELETE')
                            </form>
